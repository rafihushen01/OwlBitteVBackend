...
const uploadFilesToCloudinary = async (files) => {
    const uploadedUrls = await Promise.all(files.map(async (file) => uploadoncloudinary(file.path)));
    return uploadedUrls.filter((url) => Boolean(url));
};
const hasValidObjectId = (id) => mongoose.isValidObjectId(id);
...
export const edithomebanner = async (req, res) => {
    try {
...
export const togglehomebannerstatus = async (req, res) => {
    try {
        const id = normalizeParamId(req.params.id);
        if (!id || !hasValidObjectId(id)) {
            return res.status(400).json({ message: "Invalid banner id" });
...
export const getactivehomebanners = async (_req, res) => {
    try {
...
//# sourceMappingURL=HomeBannerController.js.map